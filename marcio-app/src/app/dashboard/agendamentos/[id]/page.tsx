...
import { MessageCircle, ChevronLeft, CheckCircle2, XCircle } from 'lucide-react'
...
export default async function AgendamentoPage({ params }: Props) {
  ...
  const canClose = profile.role !== 'barbeiro' && ag.status !== 'REALIZADO' && ag.status !== 'CANCELADO'
  const canUpdateStatus = (profile.role === 'admin' || profile.role === 'recepcionista' || profile.id === ag.executor?.id)
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-4">
      ...
      {/* Back */}
      <div className="pt-2">
        <Link href="/dashboard/agenda" className="inline-flex items-center gap-1 text-sm text-offwhite/50 hover:text-offwhite transition-colors">
          <ChevronLeft size={15} />
          Agenda
        </Link>
      </div>
      ...
      <Card>
        <CardContent className="pt-4 space-y-3">
          ...
          <Row label="Origem" value={<BadgeOrigem origem={ag.origem} />} />
      ...
      <Card>
        <CardContent className="pt-4 space-y-3">
          ...
          {ag.cliente.instagram && <Row label="Instagram" value={`@${ag.cliente.instagram.replace('@', '')}`} />}
          <Link
      ...
      {ag.status === 'REALIZADO' && ag.valor_servico && (
        <Card className="border-emerald-800/40">
          <CardContent className="pt-4 space-y-3">
            <p className="text-xs text-emerald-400/70 uppercase tracking-wide font-medium">Fechamento</p>
            {ag.valor_protese && <Row label="Prótese (material)" value={formatBRL(ag.valor_protese)} />}
      ...
      {comissoes && comissoes.length > 0 && (
        <Card>
          <CardContent className="pt-4 space-y-2">
            <p className="text-xs text-offwhite/40 uppercase tracking-wide font-medium mb-3">Comissões geradas</p>
            {comissoes.map((c: any) => (
              <div key={c.id} className="flex items-center justify-between">
                ...
                <span className={`text-sm font-medium ${c.pago ? 'text-offwhite/40 line-through' : 'text-offwhite'}`}>
                  {formatBRL(c.valor)}
                </span>
              </div>
            ))}
