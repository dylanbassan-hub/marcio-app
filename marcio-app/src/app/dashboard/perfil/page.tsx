...
export default async function PerfilPage() {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-5">
        ...
        <CardContent className="pt-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-offwhite/50">{ROLE_LABEL[profile.role] ?? profile.role}</p>
            </div>
          </div>
      ...
      {(profile.comissao_aplicacao_valor > 0 || profile.comissao_manutencao_valor > 0) && (
        <Card>
          <CardContent className="pt-4 space-y-3">
            <p className="text-xs text-offwhite/40 uppercase tracking-wide">Minhas comissões</p>
            <Row label="Por aplicação" value={formatBRL(profile.comissao_aplicacao_valor)} />
            <Row label="Por manutenção" value={formatBRL(profile.comissao_manutencao_valor)} />
            {profile.is_trafego && (
