...
type ProfileRelatorios = {
  role: string
  is_trafego: boolean | null
}

type RealizadoRelatorio = {
  id: number
  origem: OrigemLead
...
export default async function RelatoriosPage({ searchParams }: PageProps) {
  ...
  const profile = profileData as ProfileRelatorios | null
  ...
  const realizados = (realizadosData ?? []) as RealizadoRelatorio[]

  // Totais gerais
  const totalServicos = realizados.reduce(
    (s, a) => s + (Number(a.valor_servico) || 0),
    0
  )

  const totalProteses = realizados.reduce(
  ...
  // Por serviço
  const porServico = realizados.reduce(
    (
      acc: Record<string, { nome: string; servicos: number; proteses: number; count: number }>,
      a
    ) => {
      ...
      acc[k].servicos += Number(a.valor_servico) || 0
      acc[k].proteses += Number(a.valor_protese) || 0
      acc[k].count++
  ...
  // Por origem
  const porOrigem = realizados.reduce(
    (acc: Record<string, { count: number; servicos: number }>, a) => {
      const k = a.origem

      if (!acc[k]) {
        acc[k] = { count: 0, servicos: 0 }
      }
  ...
  const porExecutor = realizados.reduce(
    (acc: Record<string, { nome: string; count: number; servicos: number }>, a) => {
      const k = a.executor?.id ?? 'x'

      if (!acc[k]) {
        acc[k] = {
          nome: a.executor?.nome ?? '?',
          count: 0,
          servicos: 0,
        }
      }
  ...
  const comissoesPorUsuario = (comissoes ?? []).reduce(
    (
      acc: Record<string, { nome: string; valor: number; isTrafego: boolean }>,
      c: any
    ) => {
      const k = c.user?.id ?? 'x'
      ...
      if (!acc[k]) {
        acc[k] = {
          nome: c.user?.nome ?? '?',
          valor: 0,
          isTrafego: c.user?.is_trafego ?? false,
        }
      }

      acc[k].valor += Number(c.valor) || 0
  ...
  const totalTrafegoPago = realizados
    .filter((a) => ORIGENS_PAGAS.includes(a.origem))
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="flex gap-1">
          <a href={`?mes=${mesOffset - 1}`} className="btn-ghost text-xs px-2 py-1.5">
            ← Anterior
          </a>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-xl font-syne font-bold text-offwhite mt-1">
              {formatBRL(total)}
            </p>
            <p className="text-xs text-offwhite/30">{realizados.length} atendimentos</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-gold/30 bg-gold/5">
          <CardContent className="pt-4">
            ...
            <p className="text-xl font-syne font-bold text-gold mt-1">
              {formatBRL(totalTrafegoPago)}
            </p>
            <p className="text-xs text-gold/40">
              {totalServicos > 0
                ? `${((totalTrafegoPago / totalServicos) * 100).toFixed(0)}% do serviço`
                : '—'}
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Ranking de executores</CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Comissões a pagar</CardTitle>
        </CardHeader>
